...
class ManualFrameApp(ShowBase):
    ...
    def _is_cursor_strictly_inside(self, frame):
        """Eldönti, hogy a kurzor egy frame-en belül van-e (régi logikával)."""
        ...
        # A lokális koordináták ellenőrzéséhez képest egyszerűbb a globális (calcTightBounds).
        # Ezt a funkciót csak a Drag/Resize logika használja.
    ...
    # JAVÍTOTT FÜGGVÉNY: A DirectScrolledList látható területének ellenőrzése
    def _is_mouse_over_scroll_items(self):
        """True, ha az egér a scroll lista látható részén van."""
        if not base.mouseWatcherNode.hasMouse() or not self.scroll_list:
            return False
        ...
        # A scroll lista látható részének (itemFrame) határai
        item_frame = self.scroll_list.itemFrame

        bounds = item_frame.getTightBounds()
        if not bounds:
            return False

        p1, p2 = bounds  # biztos, hogy itt p1, p2 van, mert már ellenőriztük

        # Átszámolás render2d-be
        # figyelembe kell venni az itemFrame pozícióját
        pos = item_frame.getPos(render2d)
        p1 += pos
        p2 += pos

        abs_min_x = min(p1.getX(), p2.getX())
        abs_max_x = max(p1.getX(), p2.getX())
        abs_min_y = min(p1.getZ(), p2.getZ())
        abs_max_y = max(p1.getZ(), p2.getZ())

        return abs_min_x <= mouse_x <= abs_max_x and abs_min_y <= mouse_y <= abs_max_y

    def _on_scroll_up(self):
        """Görgetés felfelé: ha az egér a scroll list felett van, scrollozza a látható itemeket."""
        if self.active_frame is not None:
            return
        
        if self._is_mouse_over_scroll_items():
            # Görgetés felfelé (elég a scrollBy -1-et hívni a Panda3D beépített logikája miatt)
    ...
    def _on_scroll_down(self):
        """Görgetés lefelé: ha az egér a scroll list felett van, scrollozza a látható itemeket."""
        if self.active_frame is not None:
            return
